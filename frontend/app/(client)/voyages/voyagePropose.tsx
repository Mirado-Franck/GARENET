...
export default function VoyagePropose() {
  ...
  const applyFilters = async () => {
    try {
      ...
      if (selectedDate) {
        filters.date = selectedDate.toISOString().split('T')[0]; // YYYY-MM-DD
      }
      ...
      <View style={styles.content}>
        ...
        <View style={styles.countContainer}>
          <Text style={styles.countText}>
            {voyages.length}{' '}
            {voyages.length > 1 ? 'voyages' : 'voyage'} trouvé
            {voyages.length > 1 ? 's' : ''}
          </Text>
        </View>
